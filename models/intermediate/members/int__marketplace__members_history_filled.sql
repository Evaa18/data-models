...
ambassador_members AS (
    SELECT
        ...
        DATE(ambassadors.dbt_valid_from) AS dbt_valid_from,
    ...
    LEFT JOIN
        ...
        AND DATE(gap_fill_created_members.dbt_valid_from) = DATE(ambassadors.dbt_valid_from)
)

SELECT
    COALESCE(gap_fill_created_members.seeker_id, activation.seeker_id) AS seeker_id,
    COALESCE(DATE(gap_fill_created_members.dbt_valid_from), DATE(activation.member_activated_at)) AS measured_at,
    gap_fill_created_members.* EXCEPT(seeker_id, dbt_valid_from),
...
LEFT JOIN
    ...
    AND DATE(gap_fill_created_members.dbt_valid_from) = DATE(activation.member_activated_at)
LEFT JOIN
    soft_deleted_members
    ON gap_fill_created_members.seeker_id = soft_deleted_members.seeker_id
    AND DATE(gap_fill_created_members.dbt_valid_from) >= DATE(user_soft_deleted_at)
LEFT JOIN
    ...
    AND DATE(gap_fill_created_members.dbt_valid_from) >= DATE(members_activation.member_activated_at)
LEFT JOIN
    ...
    AND (DATE(gap_fill_created_members.dbt_valid_from) <= DATE(member_not_activated.member_activated_at) OR member_not_activated.member_activated_at IS NULL)
LEFT JOIN
    {{ ref('base__marketplace__seekers') }} AS incomplete_profile
    ON gap_fill_created_members.seeker_id = incomplete_profile.seeker_id
    AND DATE(gap_fill_created_members.dbt_valid_from) >= DATE(seeker_profile_first_completed_at)
LEFT JOIN
...
LEFT JOIN
    ...
    AND DATE(gap_fill_created_members.dbt_valid_from) = ambassador_members.dbt_valid_from
LEFT JOIN
    nuida_appointements
    ON gap_fill_created_members.seeker_id = nuida_appointements.seeker_id
    AND DATE(gap_fill_created_members.dbt_valid_from) = appointment_or_claim_created_on_platform_at
