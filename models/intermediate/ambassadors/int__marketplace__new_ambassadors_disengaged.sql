...
new_disengaged_ambassadors AS (
    SELECT
        ambassador_id,
        COALESCE(new_ambassadors_is_iced_up, new_ambassadors_is_soft_deleted, new_ambassadors_is_restricted, new_ambassadors_is_invalidated) AS new_ambassadors_is_disengaged,
        MIN(dbt_valid_from) AS ambassador_first_disengaged_at
    FROM
        joins
    WHERE
        COALESCE(new_ambassadors_is_iced_up, new_ambassadors_is_soft_deleted, new_ambassadors_is_restricted, new_ambassadors_is_invalidated) IS NOT NULL
    GROUP BY ALL
)
